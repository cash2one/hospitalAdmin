 ...
 * @title BeanUtils
 * @description 
 * 1，兼容基本类型与包装类型（BeanCopier不支持）�?
 * 2，兼容不规范setter（BeanCopier不支持，apache的BeanUtils不支持）
 * 3，支持copy类型�?��（apache的BeanUtils不支持，spring的BeanUtils不支持）
 */
public class BeanUtils {
	...
	public static void copy(Object source, Object target) {
		try {
			...
			logger.error("Could not copy properties from source to target");
		}

	}

	private static Object getValue(Method method, Object obj, Object[] args) throws Exception {
	...
	private static void setValue(Method method, Object obj, Object[] args) throws Exception {
		if (method == null || obj == null) {
			logger.info("Method method, Object obj must not be null");
		}
