...
@Service
public class UserTemperatureRecordServiceImpl extends BaseServiceImpl<UserTemperatureRecord, Integer> implements UserTemperatureRecordService{

	@Autowired
	private UserTemperatureRecordDao userTemperatureRecordDao;
	
	@Override
	public BaseDao<UserTemperatureRecord, Integer> getBaseDAO() {
		return userTemperatureRecordDao;
	}

	@Override
	public int getCount(UserTemperatureRecord temper) {
		int count = 0;
		if(null != temper.getUserInfo()){
			Integer userId = temper.getUserInfo().getId();
			String sql = "SELECT COUNT(*) FROM user_temperature_record WHERE user_id="+userId;
			count = userTemperatureRecordDao.executeCountSql(sql);
		}
		return count ;
	}

	@Override
	public Page<UserTemperatureRecord> getUserTemperatureWithPage( Page<UserTemperatureRecord> page, UserTemperatureRecord temper,
			Params params) {
		...
		return userTemperatureRecordDao.getUserTemperatureWithPage(temper,page,staTime,edTime);
	}
