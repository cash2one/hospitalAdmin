		...
		*/
		...
		changeScore:function(){
			...
			if("basal"==currentChangeColumn){
				...
				}else if(currentValue>=100&&currentValue<=119 || currentValue>=161&&currentValue<=180){
					autoGradeInfo.finalBaseLine=currentValue;
					autoGradeInfo.finalBaseLineScore="1";
					$("#basalScore").text("1");
					$("#basalAnalysis").text("轻度过缓（100-119）或 轻度过速（161-180）");
					
				}else if(currentValue>=120&&currentValue<=160){
					autoGradeInfo.finalBaseLine=currentValue;
					autoGradeInfo.finalBaseLineScore="2";
					$("#basalScore").text("2");
					$("#basalAnalysis").text("正常(120-160)");
				}
			...
			//振幅变异
				...
				if(currentValue<5){
					...
					$("#bpmVScore").text("0");
					$("#bpmVarAnalysis").text("过小（<5）");
					
				}else if(currentValue>=5&&currentValue<=9 || currentValue>25){
					autoGradeInfo.finalBpmVar=currentValue;
					autoGradeInfo.finalBpmVarScore="1";
					$("#bpmVScore").text("1");
					$("#bpmVarAnalysis").text("较小（5-9）或 较大（>25）");
					
				}else if(currentValue>=10&&currentValue<=25){
					autoGradeInfo.finalBpmVar=currentValue;
					autoGradeInfo.finalBpmVarScore="2";
					$("#bpmVScore").text("2");
					$("#bpmVarAnalysis").text("正常（10-25）");
				}
				
				//添加校验
				if(""==currentValue){
        			AutoScore.popup(true,"请输入变异振幅！",this);
				}else if(!reg.test(currentValue)){
					AutoScore.popup(true,"变异振幅请输入1-4位数字！",this);
				}
			...
			//周期变异
				...
				if(currentValue<3){
					...
					$("#freqVScore").text("0");
					$("#freqVarAnalysis").text("过低（<3）");
					
				}else if(currentValue>=3&&currentValue<=6){
					autoGradeInfo.finalFreqVar=currentValue;
					autoGradeInfo.finalFreqVarScore="1";
					$("#freqVScore").text("1");
					$("#freqVarAnalysis").text("较低（3-6）");
					
				}else if(currentValue>6){
					autoGradeInfo.finalFreqVar=currentValue;
					autoGradeInfo.finalFreqVarScore="2";
					$("#freqVScore").text("2");
					$("#freqVarAnalysis").text("正常（>6）");
				}
				
				//添加校验
				if(""==currentValue){
        			AutoScore.popup(true,"请输入变异频率！",this);
				}else if(!reg.test(currentValue)){
					AutoScore.popup(true,"变异频率请输入1-4位数字！",this);
				}
			...
			//加速
				...
				//1分
					...
					$("#numAccScore").text("1");
					$("#numAccAnalysis").text("较少（1-4）");
				//2分
				}else if(currentValue>=5){
					autoGradeInfo.finalNumAcc=currentValue;
					autoGradeInfo.finalNumAccScore="2";
					$("#numAccScore").text("2");
					$("#numAccAnalysis").text("正常（>=5）");
				}
			...
			//减速
				...
				if(currentValue>=2){
					...
					$("#numDecScore").text("0");
					$("#numDecAnalysis").text(">=2");
				//2
				}else if(currentValue==1){
					autoGradeInfo.finalNumDec=1;
					autoGradeInfo.finalNumDecScore="1";
					$("#numDecScore").text("1");
					$("#numDecAnalysis").text("1");
				//2
				}else if(currentValue=="0"){
					autoGradeInfo.finalNumDec=0;
					autoGradeInfo.finalNumDecScore="2";
					$("#numDecScore").text("2");
					$("#numDecAnalysis").text("无或早减");
				}
				
				//胎动
			}else if("numFetalMove"==currentChangeColumn){
				
				var currentValue=autoGradeInfo.finalNumDec=$(this).val();
				//0
				if(currentValue<=0){
					autoGradeInfo.finalNumFetalMove=currentValue;
					autoGradeInfo.finalFetalMoveScore="0";
					$("#fetalMoveScore").text("0");
					$("#numFetalMoveAnalysis").text("没有胎动");
				//2
				}else if(currentValue>=1&&currentValue<=4){
					autoGradeInfo.finalNumFetalMove=currentValue;
					autoGradeInfo.finalFetalMoveScore="1";
					$("#fetalMoveScore").text("1");
					$("#numFetalMoveAnalysis").text("较少（1-4）");
				//2
				}else if(currentValue>=5){
					autoGradeInfo.finalNumFetalMove=currentValue;
					autoGradeInfo.finalFetalMoveScore="2";
					$("#fetalMoveScore").text("2");
					$("#numFetalMoveAnalysis").text("正常（>=5）");
				}
				
				//添加校验
				if(""==currentValue){
        			AutoScore.popup(true,"请输入胎动次数！",this);
				}else if(!reg.test(currentValue)){
					AutoScore.popup(true,"胎动次数请输入1-4位数字！",this);
				}
			...
			+parseInt(autoGradeInfo.finalNumAccScore)+parseInt(autoGradeInfo.finalNumDecScore)+parseInt(autoGradeInfo.finalFetalMoveScore);
		...
		initOldScore:function(){
			...
			//胎动次数
			autoGradeInfo.autoNumFetalMove=$("[name='numFetalMove']").val();
			autoGradeInfo.finalNumFetalMove=$("[name='numFetalMove']").val();
			...
			//总分
			autoGradeInfo.autoResult=$("#sourceScore").text();
			autoGradeInfo.finalResult=$("#sourceScore").text();
