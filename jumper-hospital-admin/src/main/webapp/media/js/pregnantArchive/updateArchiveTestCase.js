$(function(){
	//必查项目
	var willCheckProject=$("#willCheckProjectData").val();
	if(willCheckProject!=null && willCheckProject!=""){
		var willCheckProjects=willCheckProject.split(",");
		$("#willCheckProjectTable tr td").find("input[type='checkbox']").each(function(){
			for(var i=0;i<willCheckProjects.length;i++){
				if($(this).val()==willCheckProjects[i]){
	...
	//备查项目
	var referenceProject=$("#referenceProjectData").val();
	if(referenceProject!=null && referenceProject!=""){
		...
		if(referenceProjects!=null && referenceProjects!=""){
			$("#referenceProjectTable tr td").find("input[type='checkbox']").each(function(){
				for(var i=0;i<referenceProjects.length;i++){
					if($(this).val()==referenceProjects[i]){
						$(this).attr("checked","checked");
					}
				}
			});
	...
	//修改初检信息
	$("#updateArchiveTestCaseBtn").bind("click",function(){
		mergerData();
		$("#updateArchiveTestCaseForm").submit();
	});
});
...
function loadTestCaseDiagnoseModelName(){
	var id=$("#testCaseDiagnoseIdData").val();
	var diagnoseModelContent=$("#diagnoseModelContentData").val();
	$.ajax({
		url:"queryAllTescaseDiagnose",
		data:{},
		type:"post",
		success:function(data){
			...
			if(data!=null){
				for(var i=0;i<data.length;i++){
					if(id==data[i].id){
						html+="<option value="+data[i].id+" selected>"+data[i].diagnoseModelName+"</option>";
					}else{
						html+="<option value="+data[i].id+">"+data[i].diagnoseModelName+"</option>";
					}
				}
			}
			$("#diagnoseModelContent").val(diagnoseModelContent);
