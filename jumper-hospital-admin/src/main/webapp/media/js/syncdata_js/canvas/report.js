	...
	return {
		...
		musicPlayer : function(){
			...
			if (media.paused) {
				...
				media.play();
				isFirst = false;
				$(this).attr("src", baseURL+"/media/image/syncdata_img/2.png");
            }else{
                media.pause();
                clearInterval(timer);
                isFirst = true;
                $(this).attr("src", baseURL+"/media/image/syncdata_img/3.png");
            }
		},
		...
		generateReport : function(){
			...
			App.confirm_show("", "确认生成报告？", function(){
				...
				$.ajax({
                    ...
                    data : {'id' : id, 'reason' : reason, 'offset' : offsetX},
                    dataType : "json", 
                    timeout : 60000,
