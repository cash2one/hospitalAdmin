	...
	return {
		...
		init : function(element, userId){
            ...
            canvasObj.ondblclick = function(event){
            	 //判断当前医院是否是义乌妇幼，如果是、不弹出对话框
            	if(hospitalId != 8569){
            	if(userId != 0){
            		var userInfo = $("#"+element).find("ul").html();
            		var hospital_id = $("#hospitalId").val();
            		var user_name = $("#"+element).find("ul").children().eq(2).text();
            		var orderId = $("#"+element).attr("order");
					...
					//var src = chatURL+"/chat/1/1/0/"+orderId +"/"+monitorTimes+"/"+hospital_id+"/"+userId+"/"+user_name;
               		...
               		+"&&toUserId="+userId+"&toNickName="+user_name+"&toHeadUrl&toUserType=2&userType=1&busCode="+busCode+"&color="+color+"&consultantId="+userId;
            	...
            	$("#modal-detail").css("overflow-y","auto");
            }
            }
		},
		chartInit : function(element){
            ...
            uterusChart.width = this.getCanvasWidth();
            uterusChart.height = this.getUterusHeight();
            var uterusContext = uterusChart.getContext('2d');
            this.drawUterus(uterusContext, element);
            
            /** 宫缩Y轴坐标 **/
            var uyLabel = $("."+element+" .uterusYLabel")[0];
            uyLabel.width = this.getLabelWeight();
            uyLabel.height = this.getUterusHeight();
            var uyContext = uyLabel.getContext('2d');
            this.drawUterusLabelY(uyContext);
		},
		...
		randerTimeAndCurrentValue : function(element, ox, dataArray, fetalMoveArray){
		    ...
		    if(fetalMoveArray != '' && fetalMoveArray.length > 0){
		    	...
		    	$("#"+element).find("li.fetalMoveNow").text("--");
		    }
		},
		...
		drawUterus : function(context, element){
			Leon.drawUterusXLine(context);
			Leon.drawUterusYLine(context, element);
			Leon.randerUterusValue(context, element, Leon.getOffsetX(element));
		},
		...
		drawUterusYLine : function(context, element){
			...
			if(Leon.getOffsetX(element) >= 0){
				for(var j = 0;j <= required_horizontal;j++){
					...
					context.moveTo(ox, 0);
					context.lineTo(ox, uterus_height);
		            context.strokeStyle = colorSelect['line'];
		            context.stroke();
		            context.closePath();
				}
				...
				for(var j = 0;j <= required_horizontal;j++){
					...
					context.lineTo(ox, uterus_height + 0.5);
		            context.strokeStyle = colorSelect['line'];
		            context.stroke();
		            context.closePath();
				}
            ...
            context.beginPath();
			context.moveTo(960, 0.5);
			context.lineTo(960, canvas_height + 0.5);
            context.strokeStyle = colorSelect['line'];
            context.stroke();
            context.closePath();
		},
		randerUterusValue : function(context, element, ox){
			var data = $($("#"+element).find(".uterusData")).val();
