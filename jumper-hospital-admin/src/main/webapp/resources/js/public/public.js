...
setTimeout(function(){
	...
	console.log("denglu111111111111111111111111111111111");
},10000)
//总数
var msgCount = 0;
//业务代码
...
function initUnreadMsgCount1(){
    ...
    console.log("initUnreadMsgCount1")
    console.log(sess);
    console.log(sessMap);//获取消息对象
    //写到标签里面  提示总数
    var aI = [];
    for (var j in sessMap) {//遍历新消息
    	...
    	console.log(keyMap);
    	//获得消息ID
    	busCode = keyMap['busCode'];
    	consultantId = keyMap['consultantId'];
    	//需要去取用户名
    	//获取touserid 或者
    	var userId= aI[j]._impl.id.substr(aI[j]._impl.id.lastIndexOf("_")+1,aI[j]._impl.id.length);
    	//获取发送用户昵称
    	...
    	var x = addList(username==0?xName:username,hospitalId,userId,username==0?xName:username,hospitalName);
    	//在提示消息弹出层
...
function addList(message,hospitalId,userId,toNick,hospitalName){
	var liList = '<li title='+hospitalName+' hospitalId = '+hospitalId+' userId = '+userId+' toNick = '+toNick+'  ><a href="#" >'+message+'</a></li>';
	return liList;
}
...
$(document).on("click","#dropdown-menuTest li",function(){
	...
	var src = imURL+"fromUserId="+ hospital_id +"&fromNickName="+hospitalName+"&fromHeadUrl&fromUserType="+fromUserType
		+"&toUserId="+userId+"&toNickName="+user_name+"&toHeadUrl&toUserType=2&userType=1&busCode="+busCode+"&color="+colorNum+"&consultantId="+consultantId;
		
	$('#messageModalLabel').html("正在和【" +user_name +"】对话中");
...
function rightMessage(opt){
	var message = '<div class="rightM" id="'+opt.msgId+'"><p class="timeM">'+opt.time+'</p><div class="centerM"><span class="nameM" title="'+userNickName+'">'+userNickName+'</span><p class="pointM"></p><i class="fr"><img src="'+userHeadImg+'"></i><p class="messageB fr">'+opt.messageBody+'</p></div></div>';
  return message;
}
function leftMessage(opt){
	var message = '<div class="leftM" id="'+opt.msgId+'"><p class="timeM">'+opt.time+'</p><div class="centerM"><span class="nameM" title="'+opt.niceName+'">'+opt.niceName+'</span><p class="pointM"></p><i class="fl"><img src="'+recveImg+'"></i><p class="messageB fl">'+opt.messageBody+'</p></div></div>';
	return message;
}

function rightImg(opt){
  var message = '<div class="rightM" id="'+opt.msgId+'"><p class="timeM">'+opt.time+'</p><div class="centerM"><span class="nameM" title="'+userNickName+'">'+userNickName+'</span><p class="pointM"></p><i class="fr"><img src="'+userHeadImg+'"></i><p class="messageB fr"><img class="showImgIm" data-src="'+opt.src1+'" data-width="'+opt.width1+'" data-height="'+opt.height1+'" src="'+opt.src+'" style="width: '+opt.width+'px;height: '+opt.height+'px;"></p></div></div>';
  return message;
}
function leftImg(opt){
  var message = '<div class="leftM" id="'+opt.msgId+'"><p class="timeM">'+opt.time+'</p><div class="centerM"><span class="nameM" title="'+recveNickName+'">'+recveNickName+'</span><p class="pointM"></p><i class="fl"><img src="'+recveImg+'"></i><p class="messageB fl"><img class="showImgIm" data-src="'+opt.src1+'" data-width="'+opt.width1+'" data-height="'+opt.height1+'" src="'+opt.src+'" style="width: '+opt.width+'px;height: '+opt.height+'px;"></p></div></div>';
  return message;
}
...
function leftFriend(opt){
	var html='<li id="'+opt.id+'"><img class="fl" src="'+opt.src+'" alt=""><span class="imUserName fl">'+opt.userName+'</span><p class="xdd fl">'+opt.wdMessage+'</p></li>';
	return html;
}
/*发送按钮*/
...
function getFriendList(name,pageNo,pageSize){
	...
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		data:{"busCode":busCode,"imAccount":name,"pageNo":pageNo,"pageSize":pageSize},
		success : function(json) {
				if(json.msg == 1){
					...
					selToID = $("#friendList").children().eq(0).attr("id").substring(3);
...
function getHistory(oldBoolean,msgId,pageNo,pageSize){
	...
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		data:{"sender":tName,"recevrer":selToID,"busCode":busCode,"startTime":"","endTime":"","pageNo":pageNo,"pageSize":pageSize,"msgId":msgId, "consultantId":consultantId},
		success : function(json) {
			if(json.msg == 1){
