...
<link
	href="${pageContext.request.contextPath}/media/css/x_report.css?${v }"
	rel="stylesheet">
<script type="text/javascript"
	src="${pageContext.request.contextPath}/media/js/xixiang/x_chart.js?${v }"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/media/js/xixiang/x_report.js?${v }"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/media/js/xixiang/drag.js?${v }"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/media/js/warning.js?${v }"></script>
<style type="text/css">
...
<c:if test="${!open }">
	<div class="col-xs-12 col-sm-9">
		暂未开通胎心监护服务，立即前往医院管理<a
			href="${pageContext.request.contextPath}/rmService/index">开通服务？</a>
	</div>
</c:if>
<c:if test="${open }">
	<div class="col-xs-12 col-sm-9">
		<form action="${pageContext.request.contextPath}/report/audit"
			method="POST" id="form">
					...
					<div class="panel-body">
						<div class="panel-head">
							<ul>
								...
								<li
									style="width: 10%; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${consumer.userName }</li>
								...
								<li style="width: 10%;"><img
									value="${consumer.consumerId }" class="play"
									src="${pageContext.request.contextPath}/media/image/button_play_hover.png"></li>
								<!-- 下面的设置只针对义务妇幼显示设置 -->
								<c:set var="YWFY_HOSPITAL_ID" value="<%=Const.YWHOSPITAL_ID%>" />
								<c:if test="${hospitalInfo.id == YWFY_HOSPITAL_ID}">
									<li style="width: 10%;"><button type="button"
											class="btn btn-info warning_info"
											userId="${consumer.userId }"
											consumerId="${consumer.consumerId }"
											hospitalId="${hospitalInfo.id}">预警记录</button></li>
								</c:if>
							...
							<tr>
								...
								<td>
									<table class="result">
										<tr>
											<td>结果：</td>
											<td colspan="5">Kreb 'S评分&nbsp;&nbsp;&nbsp;&nbsp; <input
												type="text" name="source" class="source_input" value=""
												style="width: 70px" /> 分
												<button type="button" class="btn btn-info automaticScoring"
													style="margin-left: 130px;" consumerId="${consumer.consumerId}">自动评分</button>
											</td>
										</tr>
										<tr>
											<td>NST</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="有反应" />有反应
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="无反应" />无反应
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="正弦曲线" />正弦曲线
													</label>
												</div>
											</td>
											<td colspan="2">
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="不满意1" />不满意
													</label>
												</div>
											</td>
										</tr>
										<tr>
											...
											<td style="width: 20%">
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="OST或NS-CST" />CST或NS-CST
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="阳性" />阳性
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="阴性" />阴性
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="可疑" />可疑
													</label>
												</div>
											</td>
											<td>
												<div class="checkbox">
													<label style="font-weight: normal;"> <input
														type="checkbox" name="result_item" value="不满意2">
														不满意
													</label>
												</div>
											</td>
										...
										<tr>
											<td>备注：</td>
											<td colspan="5"><textarea rows="2" cols="60"
													class="descContent" name="remark"></textarea></td>
										</tr>
										<tr>
											<td colspan="3">监护医生：${admin.name }</td>
											<td colspan="3">日期：${consumer.heart.time }</td>
										</tr>
										<tr>
						...
						<div class="table" style="text-align: left;">
							走纸速度： <select class="speed" id="${consumer.consumerId }">
						...
						<div class="operate-btn">
							<c:if test="${cloudVsit == true}">
								<button type="button" class="btn btn-success call_cont"
									...
									userName="${consumer.userName }" userId="${consumer.userId }"
									hospitalId="${hospitalInfo.id}">电话沟通</button>
							&nbsp;&nbsp;&nbsp;&nbsp; 
							</c:if>
							<c:set var="YWFY_HOSPITAL_ID" value="<%=Const.YWHOSPITAL_ID%>" />
							<!-- 去掉义乌的即时问诊 -->
							<c:if
								test="${consumer.remoteType != '院内监护' && hospitalInfo.id!=YWFY_HOSPITAL_ID }">
								<button type="button" class="btn btn-danger report-chat"
									...
									consumerId="${consumer.consumerId }"
									hospitalName="${admin.hospitalInfo.name}"
...
<!-- 聊天窗口 -->
<div class="modal fade" id="reportChatModal" tabindex="-1" role="dialog"
	aria-labelledby="reportChatModalLabel" aria-hidden="true">
			...
			<div class="modal-header">
				...
				<center>
					<h4 class="modal-title" style="font-weight: bold"
						id="reportChatModalLabel"></h4>
				</center>
			</div>
			<div class="modal-body">
				<iframe id="hospitalChatIframe" width="570px" height="600px"
...
<div class='box box-3 call_hide'>
	...
	<div class="call-ifre1">
		...
		<iframe id="callToPatient" src=""
			style="width: 290px; height: 144px; border: 0; border-radius: 4px;"
			scrolling="no"> </iframe>
	</div>
</div>
